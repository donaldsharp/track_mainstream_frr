...
def download_artifacts_recursive(artifact_url, output_dir, indent=2):
    ...
    for row in rows:
        ...
        if icon:
            ...
            if (
                "aui-iconfont-folder-filled" in icon_classes
                or "aui-iconfont-folder" in icon_classes
            ):
                is_directory = True
...
def parse_build_page(build_url):
    ...
    try:
        ...
        stage_sections = soup.find_all(
            ["div", "section", "li"],
            class_=lambda x: x and "stage" in str(x).lower() if x else False,
        )
                ...
                for link in job_links:
                    ...
                    if job_name and "/browse/" in job_url:
                        ...
                        basic_tests_jobs.append(
                            {"name": job_name, "key": job_key, "url": job_url}
                        )
                ...
                if any(pattern in job_text for pattern in known_patterns):
                    if "/browse/" in href and job_text:
                        ...
                        if job_key not in seen_keys and job_key != build_key:
                            ...
                            basic_tests_jobs.append(
                                {"name": job_text, "key": job_key, "url": job_url}
                            )
...
def main():
    ...
    while i < len(sys.argv[1:]):
        ...
        if arg in ["--list-jobs", "--list", "-l"]:
            ...
            print(
                "  ./download_test_logs.py https://ci1.netdef.org/browse/FRR-PULLREQ3-12091"
            )
            print()
            print("  # List all jobs without downloading")
            print(
                "  ./download_test_logs.py --list-jobs https://ci1.netdef.org/browse/FRR-PULLREQ3-12091"
            )
            print()
    ...
    if not build_url:
        ...
        print("Usage: ./download_test_logs.py [options] <build_url>")
        print(
            "Example: ./download_test_logs.py https://ci1.netdef.org/browse/FRR-PULLREQ3-12091"
        )
        print()
